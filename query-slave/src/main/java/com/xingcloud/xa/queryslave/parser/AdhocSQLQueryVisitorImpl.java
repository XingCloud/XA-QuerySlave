...
//
  ...
  private NamedExpression[] changeToNamedExpressions(List<LogicalExpression> logicalExpressions) {
    ...
    for (LogicalExpression logicalExpression : logicalExpressions) {
      if (logicalExpression instanceof FieldReference) {
        NamedExpression namedExpression = new NamedExpression(logicalExpression, (FieldReference) logicalExpression);
        namedExpressions.add(namedExpression);
      } else if (logicalExpression instanceof FunctionCall) {
        LogicalExpression arg = ((FunctionCall) logicalExpression).args.get(0);
        ...
        if (arg instanceof SchemaPath) {
          newFieldRef = new FieldReference(functionName + "." + ((SchemaPath) arg).getPath());
        } else if (arg instanceof FunctionCall) { //count(distinct substring(deu.row, 0, 3))
          String subFunctionName = ((FunctionCall) arg).getDefinition().getName();
          SchemaPath arg1 = (SchemaPath)((FunctionCall)arg).args.get(0);
          newFieldRef = new FieldReference(functionName + "." + subFunctionName +"." + arg1.getPath());
        } else {//count(0)  
          newFieldRef = new FieldReference(functionName + "." + ((ValueExpressions.LongExpression) arg).getLong());
        }
        NamedExpression namedExpression = new NamedExpression(logicalExpression, newFieldRef);
        namedExpressions.add(namedExpression);
      }
    }
