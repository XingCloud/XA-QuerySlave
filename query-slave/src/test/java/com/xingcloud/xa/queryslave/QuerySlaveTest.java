...
//  }
    ...
    public void testSql() throws Exception {
        String sql4 = "SELECT count(distinct fix_sof-dsk.uid) ,fix_sof-dsk.language FROM fix_sof-dsk   WHERE fix_sof-dsk.register_time>=20130228180000 and fix_sof-dsk.register_time<=20140101000000  group by fix_sof-dsk.language";
        QuerySlave querySlave = new QuerySlave();
        MapWritable mapWritable = querySlave.query(sql4);
        for (MapWritable.Entry<Writable, Writable> entry : mapWritable.entrySet()) {
            Text key = (Text) entry.getKey();
